...
export function Terminal() {
  ...
  const [status, setStatus] = useState<"idle" | "executing" | "transmitted">("idle");

  const handleExecution = (e: React.FormEvent) => {
    e.preventDefault();
    if (!email || !command) return;
        ...
        >
          ...
          <div className="p-6 md:p-8 font-mono text-sm md:text-base text-primary">
             ...
             <form onSubmit={handleExecution} className="space-y-6">
                ...
                {/* Command Prompt */}
                <div className="flex flex-col gap-2">
                  <label htmlFor="command" className="flex items-center gap-3">
                    <span className="text-destructive font-bold">~</span>
                ...
                <div className="pt-4 flex items-center justify-between border-t border-border/20">
                  <span className={`text-xs uppercase tracking-widest ${status === 'executing' ? 'text-yellow-500 animate-pulse' : status === 'transmitted' ? 'text-green-500' : 'text-muted-foreground/50'}`}>
                    {status === 'idle' && 'AWAITING_INPUT'}
                    {status === 'executing' && 'ENCRYPTING_PAYLOAD...'}
                    {status === 'transmitted' && 'TRANSMISSION_SUCCESS'}
                  </span>

                  <button 
                    type="submit"
                    disabled={status !== "idle"}
                    className="group relative px-6 py-2 bg-transparent border border-primary/30 text-primary hover:bg-primary hover:text-primary-foreground transition-all uppercase tracking-[0.2em] text-xs"
                  >
                    <span className="relative z-10 flex items-center gap-2">
                      Run <span className={`w-1.5 h-1.5 rounded-full ${status === 'idle' ? 'bg-destructive/50 group-hover:bg-background group-hover:animate-ping' : 'bg-transparent'}`} />
                    </span>
                    {/* Hover Glow */}
